...
void file_delete(char filename[], char delName[])
{
	...
	// 조사식 :원하는 변수를 직접 입력해서 디버깅 (디버그 탭 -> 조사식)
	ADDR ad = { 0 };
	ADDR del_arr[200] = { 0 };
	int count = 1;
	int check = 0;
	int i = 0;
	while (fread(&ad, sizeof(ad), 1, fp) == 1) {
		if (strcmp(ad.name, delName) != 0) { // (리턴값 비교) 삭제할 이름이 아니라면
			del_arr[i].id = ad.id;
	...
	for (int i = 0; i < 200; i++) {
		...
		if (del_arr[i].id != 0) break; // 무의미하게 200번 돌 필요 없음.
		// 0과 같다면 탈출 (배열에 데이터가 없다면 반복문 탈출)
...
void file_delete2(char filename[], char delName[])
{
	...
	// 팀작업 중 버그 수정했으면 날짜와 이메일 남기기. (사이드 이펙트)
	while (fread(&ad, sizeof(ad), 1, fp) == 1) {
		if (check == 0 && strcmp(ad.name, delName) != 0) { // 삭제할 이름이 아니라면
			del_arr[i].id = ad.id;
			strcpy(del_arr[i].name, ad.name);
			strcpy(del_arr[i].addr, ad.addr);
			strcpy(del_arr[i].tel, ad.tel);
