...
# 라벨 카테고리 매핑 설정 (이 예시에서는 6개 라벨을 사용합니다)
label_mapping = {
    "caution_zone": {"id": 1, "color": "#2b234c"},
    "roadway": {"id": 2, "color": "#a25bbf"},
    "braille_guide_blocks": {"id": 3, "color": "#a30bbf"},
    "alley": {"id": 4, "color": "#ff6037"},
    "sidewalk": {"id": 5, "color": "#fafa37"},
    "bike_lane": {"id": 6, "color": "#32b7fa"}
}

# categories 섹션 자동 추가
for label, props in label_mapping.items():
    category_info = {
        "id": props["id"],
        "name": label,
        "color": props["color"],
        "type": "any",
        "attributes": []  # 비어있는 attributes
    }
...
for image in root.findall("image"):
    ...
    for polygon in image.findall("polygon"):
        ...
        annotation_info = {
            ...
            "category_id": label_mapping.get(label, {}).get("id", 0),  # 미정의된 라벨은 0으로 설정
